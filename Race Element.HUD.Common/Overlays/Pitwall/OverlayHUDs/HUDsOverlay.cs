...
internal sealed class HUDsOverlay : CommonAbstractOverlay
{
    ...
    public sealed override void Render(Graphics g)
    {
        ...
        CommonHuds.ActiveOverlays.ForEach(hud =>
        {
            long pixels = (long)(hud.Scale * hud.Width * hud.Height);
            pixelsPerSecond += (long)(pixels * hud.RefreshRateHz);
            totalPixels += pixels;
        });
        _panel.AddLine($"  - {CommonHuds.ActiveOverlays.Count} HUDs -  ", $"Pixels: {totalPixels}, Per Second: {pixelsPerSecond}");
