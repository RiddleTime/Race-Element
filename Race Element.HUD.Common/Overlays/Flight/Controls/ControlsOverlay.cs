...
internal sealed class ControlsOverlay : CommonAbstractOverlay
{
    ...
    private const int BaseRudderBarY = 128;
    private const int BaseRudderBarWidth = 90;
    private const int BaseRudderBarHeight = 16;
    ...
    private readonly struct Model(double Aileron, double Elevator, double Rudder);
    ...
    {
        _model = new Model(0.75, 0.5, 0.25);
    }
    ...
    {
        double aileron = SimDataProvider.LocalPlane.Controls.AileronPosition;
        double elevator = SimDataProvider.LocalPlane.Controls.ElevatorPosition;
        double rudder = SimDataProvider.LocalPlane.Controls.RudderPosition;
        _model = new Model(aileron, elevator, rudder);
        ...
        int centerX = (int)(Width * Scale) / 2;
        int joystickY = (int)(BaseJoystickY * Scale);

        // === Joystick ===
        int stickX = (int)(centerX + aileron * (_joystickSize * 0.48));
        int stickY = (int)(joystickY + elevator * (_joystickSize * 0.48));

        using Pen stickPen = new(Color.FromArgb(240, 255, 255, 255), 4f * Scale);
        g.DrawLine(stickPen, centerX, joystickY, stickX, stickY);

        _cachedStickHead?.Draw(g, new Point(stickX - (int)(9 * Scale), stickY - (int)(9 * Scale)));

        // === Rudder Bar with Clipping ===
        int rudderY = (int)(BaseRudderBarY * Scale);
        int rudderBarX = (int)(centerX - _rudderBarWidth / 2f);

        int fillWidth = (int)(rudder * (_rudderBarWidth / 2f));
